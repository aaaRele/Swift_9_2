...
class ViewController: UIViewController {
    ...
    //label用于保存用户输入的用户名和密码，v用于改变背景颜色进行交互
    var v: UIView!
    var label: UILabel!
    ...
    //交互使用弹出一个提示框，让用户输入用户名和密码，然后在界面上显示输入的用户名和密码
    ...
    ///实现ActionSheet的要求，我们这里使用其弹出几个选择颜色的按钮，用户点击一个按钮将视图的背景颜色更改为相应的颜色
